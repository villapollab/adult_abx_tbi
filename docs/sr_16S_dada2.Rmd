...
To further understand the microbes within our sample, we used nanopore sequencing with the SQK-RAD004 kit and R9.4.1 flow cells (long-read shotgun sequencing). The samples chosen were the fecal DNA from a cage of mice in the vehicle group and a cage of mice in the antibiotics group.
...
# 1. Read data
#----------------------------
df_veh <- fread("/condo/neurobiome/tmhagm8/abx_depl/meta_workdir/both_sqm/veh_depl_seqscreen/functional_annotation/functional_assignments/functional_results.txt", check.names = FALSE, header = TRUE)
df_abx <- fread("/condo/neurobiome/tmhagm8/abx_depl/meta_workdir/both_sqm/abx_depl_seqscreen/functional_annotation/functional_assignments/functional_results.txt", check.names = FALSE, header = TRUE)
...
# Rename go column
df_combined <- rename(df_combined, go_id = go)
...
# 3. Normalization
#----------------------------
# Add the read counts calulcated from coverm_abundances.log here
read_depths <- data.frame(
  sample     = c("vehicle", "antibiotics"),
  total_reads = c(2768561, 4024478)
)

# Join read-depth info to your df_counts
df_counts_norm <- df_counts |>
  left_join(read_depths, by = "sample") |>
  # Create normalized counts, for example "hits per million reads":
  mutate(num_hits_norm = (num_hits / total_reads) * 1e6)

#----------------------------
# 4. Plot normalized counts
#----------------------------
...
# Plot
ggplot(df_counts_norm, aes(x = group, y = num_hits_norm, fill = sample)) +
  geom_col(position = "dodge") +  # side-by-side bars
  labs(
    x = "SCFA Group",
    y = "Number of Hits Per Million Reads",
    fill = "Sample",
...
#ggsave("seqscreen_scfa_goterm_norm_counts.png", height = 5, width = 7, units = "in")

```

![](assets/img/depl_seqscreen_scfa_v3.png)
...
Now let's plot the lemur abundance data for each metagneomic group using this R script. This plots the taxa that are 2% or more abundant in each sample and moves all the rest ot the "Other" taxa.
...
# Read VEH
veh_lem_all <- read.delim("/condo/neurobiome/tmhagm8/abx_depl/meta_workdir/both_sqm/veh_depl_lemur/relative_abundance.tsv",
  check.names = FALSE, header = TRUE) |>
  select(species, F)

# Read ABX
abx_lem_all <- read.delim("/condo/neurobiome/tmhagm8/abx_depl/meta_workdir/both_sqm/abx_depl_lemur/relative_abundance.tsv",
  check.names = FALSE, header = TRUE) |>
  select(species, F)

# Label each data set by sample
veh_lem_all$sample <- "VEH"
abx_lem_all$sample <- "ABX"

# Combine datasets
df_all <- bind_rows(veh_lem_all, abx_lem_all)

# Collapse taxa below 2% into "Other" for each sample
df_collapsed <- df_all %>%
  group_by(sample) %>%
  mutate(species = if_else(F < 0.02, "Other", species)) %>%
  group_by(sample, species) %>%
  summarise(F = sum(F), .groups = "drop")

# Calculate total abundance across samples for species
df_totals <- df_collapsed %>%
  filter(species != "Other") %>%
  group_by(species) %>%
  summarise(total_abundance = sum(F), .groups = "drop") %>%
  arrange(total_abundance)

# Now we have a species_order vector sorted by ascending total abundance
species_order <- df_totals$species

# Make "Other" the *first* factor level
df_collapsed$species <- factor(
  df_collapsed$species,
  levels = c("Other", species_order)
)

# VEH group first
df_collapsed$sample <- factor(
  df_collapsed$sample,
  levels = c("VEH", "ABX")  # vehicle first, then antibiotics
)

# Plot 
plot_stacked <- ggplot(df_collapsed, aes(x = sample, y = F, fill = species)) +
  geom_col(position = position_stack(reverse = FALSE)) +
  labs(
    x = "Sample",
    y = "Relative Abundance",
    fill = "Species",
    title = "VEH & ABX Groups at Species Level"
  ) +
  # Use ggprism theme & color scale
  theme_prism(base_size = 14) +
  scale_fill_prism(palette = "colors") +
  # Force y-axis to go 0 -> 1 and label as percentages
  scale_y_continuous(
    limits = c(0, 1),
...
#ggsave("mgx_lemur_relabunds.pdf", plot = plot_stacked, height = 5, width = 7)
```

![](assets/img/mgx_lemur_relabunds.png)
